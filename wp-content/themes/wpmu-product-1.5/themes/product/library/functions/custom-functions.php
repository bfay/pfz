...
function wpbreadcrumbs(){
	if (is_page() && !is_front_page() || is_single() || is_category()) {
	        ...
	        echo '<ul class="breadcrumbs">';
	        echo '<li class="front_page"><a href="'.get_bloginfo('url').'">'.get_bloginfo('name').'</a></li>';
	        echo '<li class="current">'.__( 'Home', TEMPLATE_DOMAIN ).'</li>';
	        echo '</ul>';
	    }
}
...
function bpbreadcrumbs(){
	if (is_page() && !is_front_page() || is_single() || is_category()) {
		...
		if (bp_is_page( BP_ACTIVITY_SLUG )){
			 echo '<li>'.__( 'Activity', TEMPLATE_DOMAIN ).'</li>';
		}
		if (bp_is_page( BP_MEMBERS_SLUG ) || bp_is_member()){
			 echo '<li class="current">'.__( 'Members', TEMPLATE_DOMAIN ).'</li>';
		}
			if (bp_is_page( BP_GROUPS_SLUG ) || bp_is_group()){
				 echo '<li class="current">'.__( 'Groups', TEMPLATE_DOMAIN ).'</li>';
			}
				if (bp_is_page( BP_FORUMS_SLUG )){
					 echo '<li class="current">'.__( 'Forums', TEMPLATE_DOMAIN ).'</li>';
				}
						if (bp_is_page( BP_BLOGS_SLUG )){
							 echo '<li class="current">'.__( 'Blogs', TEMPLATE_DOMAIN ).'</li>';
						}
						echo '</ul>';
...
function signup_button(){
	include (TEMPLATEPATH . '/library/options/options.php');
	$signupfeat_buttontext = get_option('dev_product_signupfeat_buttontext');
	$signupfeat_buttontext_custom = get_option('dev_buddyproduct_signupfeat_buttontextcustom');
