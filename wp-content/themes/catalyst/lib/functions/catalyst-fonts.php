...
add_action( 'wp_print_styles', 'catalyst_enqueue_google_fonts' );
/**
 * Enqueue Google fonts.
 *
 * @since 1.0
 */
function catalyst_enqueue_google_fonts()
{
	...
	if( defined( 'DYNAMIK_ACTIVE' ) )
	{
		...
		if( is_array( catalyst_get_dynamik_alt( 'font_type' ) ) )
		{
			foreach( $catalyst_google_font_array as $google_font => $google_font_data )
			{
				if( in_array( $google_font_data['value'], catalyst_get_dynamik_alt( 'font_type' ) ) )
				{
		...
		{
			wp_enqueue_style( 'catalyst_enqueued_google_fonts', $protocol . '://fonts.googleapis.com/css?family=' . $google_fonts );
		}
	}
	elseif( defined( 'CHILD_ACTIVE' ) )
	{
		global $catalyst_child_google_fonts;
		$catalyst_google_font_array = catalyst_google_font_array();
		$google_fonts = '';
		
		if( is_array( $catalyst_child_google_fonts ) )
		{
			foreach( $catalyst_google_font_array as $google_font => $google_font_data )
			{
				if( in_array( $google_font_data['value'], $catalyst_child_google_fonts ) )
				{
					$google_fonts .= $google_font_data['url_string'];
				}
			}
...
/**
 * Include specified Google fonts into Child Themes
 * exported by the Dynamik Child Theme.
 *
 * @since 1.0
 * @return either the Google font output or false.
 */
function catalyst_child_export_google_fonts()
{
	$catalyst_google_font_array = catalyst_google_font_array();
	$google_fonts = array();
	...
	foreach( $catalyst_google_font_array as $google_font => $google_font_data )
	{
		if( in_array( $google_font_data['value'], catalyst_get_dynamik( 'font_type' ) ) )
		{
			$google_fonts[] = $google_font_data['value'];
		}
	}

	if( !empty( $google_fonts ) )
	{
		$google_font_output = var_export( $google_fonts, true );
		return $google_font_output;
	}
	else
	{
		return false;
...
/**
 * Build the Catalyst font menu HTML.
 *
 * @since 1.0
 */
function catalyst_build_font_menu( $selected = '' )
{
	$catalyst_font_array = catalyst_font_array();
	
	foreach( $catalyst_font_array as $font_type => $fonts )
	{
...
/**
 * Create an array of Catalyst fonts.
 *
 * @since 1.0
 * @return an array of Catalyst fonts.
 */
function catalyst_font_array()
{
	$catalyst_font_array = array(
 ...
 */
function catalyst_google_font_array()
{
	$catalyst_google_font_array = array(
		"Aclonica" => array(
	...
	return $catalyst_google_font_array;
}

//end lib/functions/catalyst-fonts.php