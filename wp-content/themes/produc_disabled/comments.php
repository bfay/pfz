	<?php
		...
		if ( is_page() && !have_comments() && !comments_open() && !pings_open() )
			return;
			include( TEMPLATEPATH . '/library/functions/conditional-functions.php' );
	?>
			...
			<div class="comment-content">
				...
				<?php else : ?>
					...
					<form action="<?php echo get_option( 'siteurl' ); ?>/wp-comments-post.php" method="post" id="commentform" class="standard-form">
						<?php if ( $user_ID ) : ?>
							<p class="log-in-out">
														<?php printf(__('Logged in as %s.','bp_product'), '<a href="'.get_option('siteurl').'/wp-admin/profile.php">'.$user_identity.'</a>'); ?> <?php $mywp_version = get_bloginfo('version'); if ($mywp_version >= '2.7') { ?> <a href="<?php echo wp_logout_url(get_bloginfo('url')); ?>"><?php _e('Log out &raquo;','bp_product'); ?></a> <?php } else { ?> <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="<?php __('Log out of this account','bp_product'); ?>"><?php _e('Log out &raquo;','bp_product'); ?></a> <?php } ?>
							</p>
						<?php else : ?>
